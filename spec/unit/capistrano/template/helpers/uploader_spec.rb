...
RSpec.describe Capistrano::Template::Helpers::Uploader do
  ...
  let(:digest_cmd) { %(echo "%<digest>s %<path>s" | md5sum -c --status) }
  ...
  describe "#file_changed?" do
    it 'uses the "digest_cmd" to check file changes' do
      expect(subject).to receive(:__check__).with(%(echo "#{digest} /var/www/shared/config/database.yml" | md5sum -c --status))
  ...
  describe "#user_changed?" do
    it 'returns "false" when no user is given' do
      subject.user = nil
      expect(subject).not_to be_user_changed
    end
